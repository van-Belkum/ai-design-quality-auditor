...
# ---- MIMO options (deduped, case-normalized) ----
_MIMO_RAW = \"\"\"
...
\"\"\"  # shortened; you can paste your full list here

def _prep_mimo_options(raw: str) -> list[str]:
    items = set()
    for line in raw.splitlines():
        s = line.strip()
        if not s:
            continue
        # Normalise '8X8' -> '8x8', squash double spaces
        s = re.sub(r"X", "x", s)
        s = re.sub(r"\\s{2,}", " ", s)
        items.add(s)
    return sorted(items, key=lambda x: (x.count(\"@\"), len(x), x))

MIMO_OPTIONS = _prep_mimo_options(_MIMO_RAW)
if not MIMO_OPTIONS:
    MIMO_OPTIONS = [\"18\\21 @2x2\", \"18\\21\\26 @4x4\", \"18\\21\\26 @4x4; 3500 @8x8\"]  # fallback
...
# ---------- utils ----------
def load_rules(file) -> Dict[str, Any]:
    import yaml
...
def rule_applies(rule_when: dict, meta: dict) -> bool:
    ...
    for key, want in rule_when.items():
        ...
        if isinstance(want, list):
            # if list entries look like regex, treat as exact still (keep predictable)
        ...
        for r in rules:
            ...
            for rx in checks.get(\"require_regex\", []) or []:
                try:
                    ...
                    finds.append(Finding(\"\", pno, \"Consistency\", f\"[{r.get('id','RULE')}] Invalid regex in rule: {rx}\", []))
...
def learn_from_feedback(
    ...
    Learn from a Findings CSV labeled with 'Valid'/'Not Valid'.
      ...
      - 'Valid' on those -> ADD a rule (category = default_category) with this requirement/forbid
        conditioned by target_meta (client/project/site_type/... incl. mimo_s1/2/3).
    \"\"\"
    import yaml
    ...
    if mask_v.any():
        cond = {}
        if target_meta:
            for k in [\"client\",\"project\",\"site_type\",\"vendor\",\"cabinet_loc\",\"radio_loc\",\"mimo_s1\",\"mimo_s2\",\"mimo_s3\",\"site_address\"]:
                v=str(target_meta.get(k,\"\"))
                if v: cond[k]=[v]
...
def apply_manual_qa(manual_df: pd.DataFrame, rules_blob: dict):
    ...
    CSV columns (any can be blank):
      ...
      client,project,site_type,vendor,cabinet_loc,radio_loc,mimo_s1,mimo_s2,mimo_s3,site_address,
      category, hint, a, b
    ...
    for _, r in manual_df.fillna(\"\").iterrows():
        ...
        if rtype == \"require_text\":
            ...
            if pt:
                entry[\"checks\"][\"require_text\"] = [pt]
        elif rtype == \"require_regex\":
            rx = str(r.get(\"regex\",\"\")).strip()
            if rx:
                entry[\"checks\"][\"require_regex\"] = [rx]
        elif rtype == \"forbid\":
            pt = str(r.get(\"pattern\",\"\")).strip()
            if pt:
                entry[\"checks\"][\"forbid\"] = [{\"pattern\": pt, \"hint\": hint}]
...
# ---------- UI ----------
st.set_page_config(page_title=\"AI Design QA — v7 (single audit + MIMO S1/S2/S3)\", layout=\"wide\")
st.title(\"AI Design Quality Auditor — v7\")
st.caption(\"Single-file audits, metadata-aware rules, fast learning, and explicit MIMO S1/S2/S3.\")
...
with st.sidebar:
    ...
    st.subheader(\"Proposed MIMO\")
    mimo_same = st.checkbox(\"Use S1 value for S2 & S3\", value=True)
    mimo_s1 = st.selectbox(\"Proposed MIMO S1\", MIMO_OPTIONS, index=0 if not MIMO_OPTIONS else 0, key=\"mimo_s1\")
    if mimo_same:
        mimo_s2 = mimo_s1
        mimo_s3 = mimo_s1
        st.caption(f\"S2 = S3 = **{mimo_s1}**\")
    else:
        mimo_s2 = st.selectbox(\"Proposed MIMO S2\", MIMO_OPTIONS, index=min(1, len(MIMO_OPTIONS)-1), key=\"mimo_s2\")
        mimo_s3 = st.selectbox(\"Proposed MIMO S3\", MIMO_OPTIONS, index=min(2, len(MIMO_OPTIONS)-1), key=\"mimo_s3\")

    meta[\"mimo_s1\"] = mimo_s1
    meta[\"mimo_s2\"] = mimo_s2
    meta[\"mimo_s3\"] = mimo_s3
    meta[\"site_address\"] = st.text_area(\"Site Address\")
    ...
    st.divider(); st.subheader(\"Manual / Quick Rules → Apply to pack\")
    manual_csv = st.file_uploader(\"Manual / Quick Rules CSV\", type=[\"csv\"])  # updated header expected
    apply_manual_btn = st.button(\"Apply Manual/Quick Rules\")

    st.divider(); st.subheader(\"Learn from labeled findings\")
    fb_csv = st.file_uploader(\"Findings CSV (with Valid/Not Valid)\", type=[\"csv\",\"xlsx\")
